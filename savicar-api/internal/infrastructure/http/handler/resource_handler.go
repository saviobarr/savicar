...
import (
	...
	resourcesvc "savicar-api/internal/application/resource"
...
func NewResourceHandler(svc *resourcesvc.Service) *ResourceHandler {
	return &ResourceHandler{svc: svc}
}

func (h *ResourceHandler) RegisterRoutes(r gin.IRouter) {
	g := r.Group("/resources")
	g.GET("", h.getAll)
	g.GET("/:id", h.getByID)
	g.POST("", h.create)
