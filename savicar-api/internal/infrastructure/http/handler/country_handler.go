...
import (
	...
	countrysvc "savicar-api/internal/application/country"
	"savicar-api/internal/domain/country"
)

type CountryHandler struct {
	svc *countrysvc.Service
}

func NewCountryHandler(svc *countrysvc.Service) *CountryHandler {
	return &CountryHandler{svc: svc}
}

func (h *CountryHandler) RegisterRoutes(r gin.IRouter) {
	g := r.Group("/countries")
	g.GET("", h.getAll)
	g.GET("/:id", h.getByID)
	g.POST("", h.create)
